...
bool	Server::USER(User &user, std::string &params)
{
	...
	return this->replyPush(user, ':' + /* user.getMask() */ this->_config["host"] + " 001 " + user.getNickname() + " :Welcome to the Mine, " + user.getMask() /* + '.' */)
		&& this->replyPush(user, ':' + /* user.getMask() */ this->_config["host"] + " 002 " + user.getNickname() + " :Your host is " + this->_config["server_name"] + ", running version " + this->_config["server_version"] + '.')
		&& this->replyPush(user, ':' + /* user.getMask() */ this->_config["host"] + " 003 " + user.getNickname() + " :This server was created " + this->_creationTime + '.')
		&& this->replyPush(user, ':' + /* user.getMask() */ this->_config["host"] + " 004 " + user.getNickname() + " :" + this->_config["server_name"] + " " + this->_config["server_version"] + ' ' + User::getAvailableModes() + ' ' + Channel::availableModes() + '.')
